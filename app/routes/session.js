...
function SessionHandler(db) {
    ...
    const validateSignup = (userName, firstName, lastName, password, verify, email, errors) => {
        ...
        const PASS_RE = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9]).{8,}$/;
        ...
        if (!PASS_RE.test(password)) {
            errors.passwordError = "Password must be at least 8 characters including uppercase, lowercase, number, and special character.";
            return false;
        }
        if (password !== verify) {
